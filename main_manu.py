...
title_screen = TitleScreen("ChatGPT Image May 17, 2025, 01_28_47 PM.png", (1280, 720))
help_screen = HelpScreen((1280, 720))
...
while running:
    for event in pygame.event.get():
        ...
        if state == STATE_TITLE:
            button_clicked = title_screen.handle_event(event)
            if button_clicked == 0:  # Help
                state = STATE_HELP
            elif button_clicked == 1:  # Start
                print("Start game!")
            elif button_clicked == 2:  # Exit
                running = False
        elif state == STATE_HELP:
            if help_screen.handle_event(event):
                state = STATE_TITLE
